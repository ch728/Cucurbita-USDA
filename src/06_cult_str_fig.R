...
# Read in files
cpepo <- read.table("data/03_geno/data/admix/cpepo/cpepo_admix_cult.5.Q", sep=" ", header=F)
...
cmos <- read.table("data/03_geno/data/admix/cmoschata/cmoschata_admix_cult.5.Q", sep=" ", header=F)
...
cmax <- read.table("data/03_geno/data/admix/cmaxima/cmaxima_admix_cult.5.Q", sep=" ", header=F)
...
# Create plot
cpepoLg$class <- factor(cpepoLg$class, c("Acorn", "Crook", "Scallop",
					 "Zucchini", "Pumpkin", "Gem",
					 "Marrow", "Other"))
...
cmosLg$class <- factor(cmosLg$class, c("Neck", "Cheese", "Japonica",
				       "Calabaza"))
...
cmaxLg$class <- factor(cmaxLg$class, c("Buttercup", "Kobocha",
				       "Hubbard", "Processing",
				       "Australian", "Mammoth"))
